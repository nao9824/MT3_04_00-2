	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (isSpring == true) {
			...
			if (length != 0.0f) {
				...
				Vector3 restPosition = Multiply(spring.naturalLength, Add(spring.anchor, direction));
				Vector3 displacement = Multiply(length, Subtract(ball.position, restPosition));
				Vector3 restoringForce = Multiply(-spring.stiffness, displacement);
				//減衰抵抗を計算する
				Vector3 dampingForce = Multiply(-spring.dampingCoefficient, ball.velocity);
				Vector3 force = Add(Add(restoringForce, dampingForce), kGravity);
				ball.acceleration = { force.x / ball.mass, force.y / ball.mass, force.z / ball.mass };
			}


			ball.velocity.x += deltaTime * ball.acceleration.x;
			ball.velocity.y += deltaTime * ball.acceleration.y;
			ball.velocity.z += deltaTime * ball.acceleration.z;
		...
		Vector3 springNdcVertex = Transform(spring.anchor, worldViewProjectionMatrix);
		Vector3 springScreenVertice = Transform(springNdcVertex, viewportMatrix);
		...
		Novice::DrawLine((int)springScreenVertice.x, (int)springScreenVertice.y, (int)ScreenVertice.x, (int)ScreenVertice.y, WHITE);
